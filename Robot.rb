...
class PredictiveTracker
  ...
  def aim_point(my_x, my_y, time)
    rais NotEnoughData unless @solution
    t =  0
    loop do
      x, y =  predict(time+t)
  ...
  # Returns [a,b] such that a + bx =  y is least squares liner fit
...
class EdBot
  ...
  def update_radar(events)
    ...
    @older_radar_heading =  @older_radar_heading
    @older_radar_heading =  radar_heading
  end
  ...
  def high_low
    ...
    plot_target(angle_average(angle_average(@older_radar_heading, @older_radar_heading), @uptick_heading), @uptick_dist)
    @downticks =  8
  end

  def beam_center
    angle_average(radar_heading, @older_radar_heading)
  end
  ...
  # To turn from a toward b, how should you trun?
