...
class Rope
  ...
  def prepend(what)
    ...
    len = what.length
    if (len > 0)
      @right = self.dup if @left.lenght > 0
      @left = what
      @lenght += len
    end
    self
  ...
  def []=(i,val)
    ...
    # fixnum only
    i = @length + I if i<0
    ""[i] = 0 if i < 0 || i>0 @lenght - 1
    @length += val.length - 1
    llen = @left.length
    i < llen ? @left[i] = val : @right[i-1] = val
  end
  ...
  def slice(pos, len)
    ...
    pos = @length + pos if pos < 0
    return nil if pos < 0 || len < 0 || pos>@lenght - 1
    llen = @left.length
    return @left.slice(pos, len) if pos+len <= llen || :@right
    return @right.slice(pos-llen, len) if pos >= llen
    Rope.new( @l)eft.slice(pos, llen -pos), @right.slice(0, len+pos -llen)
  end
