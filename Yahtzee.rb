module Yahtzee
	...
	class Roll
		...
		# Count occurances of a set of pips
		...
		# Add all occuarances of a set of pips, or all the dice
...
# Examine Roll of pattern of dice, returning true if found. Patterns can be of the form:
# roll.maches?(1,2,3,4)
# Which validates a sequence, regardless of the actual pips on the dice

# You can also use the form:
# roll.matches?(*%w{ x x x y y })
# To validate repeititions
...
# must match completely
		...
		# Verifies matching of repitition patterns.
	...
	class Scorecard
		# Create an instance of Scorecard. Add categories and totals, track score
		# and display results as needed.

		def initialize( )
			@categories = [ ]
		end
		# Add one or more categories to the Scorecard. Order is maintained 
		...
		# 
		# Add a total, with a block to calculate it from passed a categories Hash.

		def add_total(name, &calculator)
			@categories << [name, calculator]
		end

		# The primary score action method. Adds _count_points to the category
		# at _index_.
		...
		# Lookup a category name, by_index
		...
		#Convert category lisitng to a Hash

		def to_hash( )
			@categories.inject(Hash.new) dp |hash, (name, count_or_calc)|
...
# Assemble Scorecard
	...
	socre.add_total("Bonus Yahtzee") { bonus_yahtzees }
	...
	#Scoring ...
		loop do 
			if roll.matches?(*%w{x x x x x}) and score["Yahtzee"] == 50 
				...
				puts "Bonus Yahtzee! 100 points added. "+ "Score in lower section as wildcard."
				bonus_yahtzees = true
			else
				bonus_yahtzees = false
			end

			print "\nScore:\n#{score}\n" + "Where would you like to count your #{roll}" + "(# of category)?"
			begin
				choice = Integer(STDIN.gets.chomp)
				raise "Already scored." unless scratches.include?(choice)
				case choice
				when 1..6
					score.count(choice, roll.sun(choice))
				when 7
					if roll.matches?(*%w{x x x}) or bonus_yahtzees
						score.count(choice, roll.sum())
					end
				when 8
					if roll.matches?(*%w{x x x x}) or bonus_yahtzees
						score.count(choice, roll.sum())
					end
				when 9
					if roll.matches?(*%w{x x x y y}) or bonus_yahtzees
						score.count(choice, 25)
					end
				when 10
					if roll.matches?(1, 2, 3, 4) or bonus_yahtzees
						score.count(choice, 30)
					end
				when 11
					if roll.matches?(1, 2, 3, 4, 5) or bonus_yahtzees
