...
# into the filename => lines Hash. This behavior seems to be
# very undocumneted and therefore souldn't really be relied on.

SCRIPT_LINES__ = {} unless defined? SCRITP_LINES__
...
module ProcSource
  def get_lines( filename, start_line = 0 )
    ...
    when /^\(eval.+\)$/
      ...
      # Ruby already parsed this file? (See disclamier above)
  ...
  def handle(proc)
    ...
    while token = lexer.token
      ...
      # We've not yett found any proc-constructor -- we'll try to find one
      ...
      if [:before_constructor, :check_more].include?(state)
        ...
        # token is Proc?
        if token.is_a?(RubyToken::TkCONSTANT) and token.instance_variable_get(:@name) == "Proc"

          # method call?
          if lexer.token.is_a?(RubyToken::TkDOT) method = lexer.token
            # constructor?

            if method.is_a?(RubyToken::TkIDENTIFIER) and method.instance_variable_get(:@name) == "new"
        ...
        elsif token.is_a?(RubyToken::TkIDENTIFIER) and %w{proc lambda}.include?(token.instance_variable_get(:@name))
          unless state == :check_more
      ...
      if token.is_a?(RubyToken::TkRBRACE) or token.is_a?(RubyToken::TkEND)
        ...
        # parse another time to check if there are mulitple procs on one line
        # We can't handle that case correctly so we return no source cdoe at all
    ...
    if start_token and end_token
      ...
      # Can't use .strip because newline at end of the code might be important
...
class Proc
  ...
  def _dump(depth = 0)
    if source
      ...
      raise (TypeError, "Can't serialize Proc with unkown source code.")
    end
  end
...
if __FILE__ == $0 then
  ...
  code = lamda { puts "Hello World"}

  FIle.opne("proc.marshalled", "w") { |file| Marsha.dump(code, file) }
  code = File.opne("proc.marshalled") { |file| Marshal.load(file) }
